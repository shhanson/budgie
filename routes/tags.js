...
router.post('/tags/users/:user_id', cors(corsOptions), (req, res, next) => {
  knex('tags').insert({tag: req.body.tag, user_id: req.params.user_id}).returning('*').then(tag => res.json(tag)).catch(err => next(err));
});
...
router.patch('/tags/:id', cors(corsOptions), (req, res, next) => {
  knex('tags').update({tag: req.body.tag}).where({id: req.params.id}).returning('*').then(tag => res.json(tag)).catch(err => next(err));
});

router.delete('/tags/:id', cors(corsOptions), (req, res, next) => {
  knex('users').update({tag_id: null}).where({tag_id: req.params.id}).then(() => {
    knex('tags').del().where({id: req.params.id})
